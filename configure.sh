...
EOF
  ...
  EDGE_DOMAIN="$(prompt_domain_value "1/4 Domain for REALITY" "${EDGE_DOMAIN}")"
  PANEL_DOMAIN="$(prompt_domain_value "2/4 Domain for Marzban panel" "${PANEL_DOMAIN}")"
  APP_DIR="$(prompt_text "3/4 Install directory" "${APP_DIR}" 1)"
  MARZBAN_IMAGE="$(prompt_text "4/4 Marzban image" "${MARZBAN_IMAGE}" 1)"
...
Before installation, make sure:
  ...
  3. Ports 80/tcp and 443/tcp are open on the server.
